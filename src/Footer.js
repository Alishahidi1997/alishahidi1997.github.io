...
import { useSitePageViews } from './hooks/useSitePageViews';


function Footer(){

  const [userInfo] = useState(userData);
  const { configured, count, loading, error } = useSitePageViews();
  ...
  <footer class="d-flex flex-wrap justify-content-between align-items-center py-3 my-4 border-top">
    <div class="col-md-4 d-flex flex-column align-items-start">
      ...
      <span class="text-muted">© 2025, Ali Shahidi</span>
      </div>
      {configured ? (
        <span class="text-muted small mt-1">
          {loading ? '…' : null}
          {!loading && error ? 'Page view stats unavailable' : null}
